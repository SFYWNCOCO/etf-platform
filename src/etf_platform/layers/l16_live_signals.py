...
def score_fund_quality(sector: str) -> Dict:
    ...
    composite = q["rating"] * 0.4 + q["manager"] * 0.35 + q["institutional"] * 0.25
    return {
...
def get_live_signals(sector: str, daily_amount_yi: float = 1.0,
                     premium_pct: float = 0.0, is_cross_border: bool = False) -> Dict:
    ...
    # Apply premium adjustment to composite (capped at +/-1.5)
    prem_adjusted = max(-1.5, min(1.5, sector_premium))

    # 综合评分: 流动性40% + 折溢价30% + 基金质量30% + 行业溢价调整
    composite = liq["score"] * 0.40 + prem["score"] * 0.30 + quality["score"] * 0.30 + prem_adjusted
